...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.knn_curvatures = torch.empty(0)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        print("KNN Curvatures Init!")
        self.knn_curvatures = self.compute_curvature(torch.tensor(np.asarray(points)).float().cuda(), 10)
        self.knn_curvatures = self.get_knn_curvatures.mean()
    ...
    @property
    def get_knn_curvatures(self):
        return self.knn_curvatures
    
    def gennerate_knn_curvatures(self):
        print("Caculating Knn Curvatures!")
        self.knn_curvatures = self.compute_curvature(self.get_anchor, 3)
    ...
    def compute_curvature(self, points, k=10): 
        ...
        for i in range(len(nppoints)):
            ...
            # 添加正则化项
            covariance_matrix += 1e-6 * torch.eye(covariance_matrix.shape[0], device=points.device)
    
            # 使用 SVD 计算特征值
            _, eigenvalues, _ = torch.svd(covariance_matrix)
        ...
        return curvatures
    def curvature_densification_mask(self):
         print("Curvature Densification!")
         self.gennerate_knn_curvatures()
         knn_curvatures = self.knn_curvatures.view(self.get_anchor.shape[0], -1)
         curvature_mask =(knn_curvatures <= 0.1).squeeze()
         curvature_mask = torch.cat([curvature_mask] * 1, dim=0)
